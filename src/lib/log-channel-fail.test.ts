...
describe("formatChannelFail (pure formatter)", () => {
  ...
  it("NETWORK: surfaces the err.code in parentheses when available", () => {
    ...
    expect(out).to.equal("Cloud REST: network error (ENOTFOUND) (loading device list) — retrying every 5 min");
  });
  ...
  it("AUTH: includes HTTP status when err is HttpError + no auto-retry hint", () => {
    ...
    expect(out).to.equal("Cloud REST: authentication failed (HTTP 401) — check adapter config, no auto-retry");
  });
  ...
  it("RATE_LIMIT: includes HTTP 429 + retry-after hint", () => {
    ...
    expect(out).to.equal("Cloud REST: rate-limited by Govee (HTTP 429) — retrying in 60 s");
  });
  ...
  it("UNKNOWN: includes err.message + retryHint", () => {
    ...
    expect(out).to.equal("Cloud REST: request failed (loading device list) — Govee returned weird payload — retrying every 5 min");
  });
  ...
  it("VERIFICATION_PENDING: directs user to Settings, no retry hint inserted", () => {
    ...
    expect(out).to.equal("Cloud REST: verification code required — open adapter Settings and request a code");
  });
});
...
describe("logChannelFail (dedup wrapper)", () => {
  it("first failure in a category goes to warn, stack goes to debug", () => {
    ...
    expect(warns[0].msg).to.include("Cloud REST: Timeout after 15000ms");
    // stack lives on debug
    expect(debugs).to.have.lengthOf(1);
    expect(debugs[0].msg).to.include("Cloud REST fail detail:");
  });
  ...
  it("second failure in same category goes to debug only", () => {
    ...
    const warns = entries.filter(e => e.level === "warn");
    expect(warns, "second failure must not produce another warn").to.have.lengthOf(1);
    // first call: 1 warn + 1 debug (stack). second: 1 debug (repeated). total: 2 debugs.
    const debugs = entries.filter(e => e.level === "debug");
    expect(debugs).to.have.lengthOf(2);
    expect(debugs[1].msg).to.include("(repeated; raw:");
  });
  ...
  it("different category after first → warn again", () => {
    ...
    expect(warns[1].msg).to.include("authentication failed (HTTP 401)");
  });
});
...
describe("logChannelRestored", () => {
  it("emits info + resets dedup state when channel had previously failed", () => {
    ...
    expect(dedup.lastCategory).to.be.null;
    const infos = entries.filter(e => e.level === "info");
    expect(infos).to.have.lengthOf(1);
    expect(infos[0].msg).to.equal("Cloud REST: connection restored");
  });
